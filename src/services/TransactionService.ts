...
export default class TransactionService {
  static transfer(
    ...
    const sourceAccounts = this.pickSourceAccounts(
      fromUser.getAccountIds(),
      sourceBankId,
    );
  ...
  private static pickSourceAccounts(
    accountIds: BankAccountId[],
    sourceBankId: BankId,
  ): BankAccount[] {
    const sourceAccounts = accountIds
      .map((accountId) => GlobalRegistry.getBankAccount(accountId))
  ...
  private static debitAcrossAccounts(
  ...
  private static pickTargetAccount(
    ...
    for (const accountId of accountIds) {
      ...
      if (
        account.getBankId() === targetBankId &&
        account.getId() !== excludedAccountId
      ) {
        return account;
      }
    }
